	...
	<div class="easy-numpad-frame" id="easy-numpad-frame">
		<div class="easy-numpad-container">
			...
			<div class="easy-numpad-number-container">
				...
				<div style="text-align:center; margin-top: -20px; font-size:0.7em;">
					<p class="invalid-feedback pin_password_em_" style="height:10px; min-height:0px;" ></p>
				</div>
			</div>
		</div>
	</div>
		...
		<div class="easy-numpad-container">
			...
			<div class="easy-numpad-number-container">
				...
				<div style="text-align:center; margin-top: -20px;">
					<p class="invalid-feedback pin_password_em_" style="height:10px; min-height:0px; font-size:0.3em;" ></p>
				</div>
			</div>
		</div>
	</div>
...
function easypinconfirm(num) {
	...
	if (pin.length == pinDigits){
		...
		if ($('#pin_password').val() != $('#pin_password_confirm').val()){
			...
			$('.pin_password_em_').text('Il Pin non è corretto!');
			return false;
		}else{
		...
		.then(function(old) {
			...
			clearAllData('pin')
				.then(function(){
					writeData('pin', post)
						.then(function() {
							console.log('Save updated pin info in indexedDB', post);
							$('#pinRequestModal').modal("hide");
							$('#pinRequestButton').text("Conferma");
						})
				})
		})
	return isPinRequest;
