...
class SendCommand extends CConsoleCommand
{
	...
	public function actionIndex($id){
		...
		$save->WriteLog('wallet-tts','commands','send',"Start : Checking tx id #: $id");

		set_time_limit(0); //imposto il time limit unlimited

		//carico l'invoice
		$tokens = $this->loadModel(crypt::Decrypt($id));
		$save->WriteLog('wallet-tts','commands','send',"Tokens loaded.");

		//Carico i parametri
		$settings=Settings::load();
		if ($settings === null){//} || empty($settings->poa_port)){
			$save->WriteLog('wallet-tts','commands','send',"Settings NOT loaded.",true);
		}
		...
		if (!$poaNode){
			$save->WriteLog('wallet-tts','commands','send',"All Nodes are down.",true);
		}
		$web3 = new Web3($poaNode);
		$contract = new Contract($web3->provider, $settings->poa_abi);
		$save->WriteLog('wallet-tts','commands','send',"Web3 connected.");
		...
		while (true)
		{
			$save->WriteLog('wallet-tts','commands','send',"$SendingType status: ".$tokens->status.", Seconds: ".$expiring_seconds);
			$ipnflag = false;

			if ($tokens->type == 'ether'){
				$web3->eth->getTransactionByHash($tokens->txhash, function ($err, $transaction){
					if ($err !== null) {
						$save->WriteLog('wallet-tts','commands','send'," : error ether: ".$err->getMessage(), true);
					}
				...
				$contract->eth->getTransactionReceipt($tokens->txhash, function ($err, $transaction)  {
					if ($err !== null) {
						$save->WriteLog('wallet-tts','commands','send'," : error token: ".$err->getMessage());
						sleep(5); //exit;
					}
					self::setTransaction($transaction);
				});
				...
				if (!empty($transaction->blockNumber)){
					if (!empty($transaction->logs)){
						...
						if (strtoupper($transaction->from) == strtoupper($tokens->from_address)){
							...
							Push::Send($save->Notification($notification),'wallet-tts');
			...
			if ($ipnflag){ //send ipn in case flag is true: può venire
				...
				// SALVA I TOKEN
				if ($tokens->update()){
					$save->WriteLog('wallet-tts','commands','send',"End   : $SendingType wallet invoice n. $id, Status: $tokens->status, Price: $transactionValue.");
					$this::sendIpn($tokens->attributes, $SendingType);

				}
				else
					$save->WriteLog('wallet-tts','commands','send',"Error : Cannot save $SendingType transaction on invoice n. $id, Status: $tokens->status.");
