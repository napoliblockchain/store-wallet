...
class BlockchainController extends Controller
{
	...
	public $maxBlocksToScan = 1500; // 200 IS FOR TESTING AT HOME 1500; //don't touch it (1500)
	...
	public function accessRules()
	{
		return array(
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array(
					'scanForTransactions', //scansiona la blockchain per nuove transazioni
	...
	*/
	public function actionScanForNew(){
		...
		if (!$poaNode){
			$save = new Save;
			$save->WriteLog('wallet-tts','blockchain','scanForNew',"All Nodes are down.");
				echo CJSON::encode(array(
					'id'=>time(),
	...
	* Questo presuppone una modifica del software Wallet & N_Pay nella fase di
	* creazione del wallet. Valore standard 0x0 = blocco 0
	* Salva in db quelle trovate
	* Aggiorna il blocco
	* La ricerca avviene in questo modo:
	*		Carica il blockNumber relativo al wallet dal db.
	*		Cerca nei blocchi successivi fino a 100 (da verificare l'impatto del timing)
	*		Se non trova nulla, aggiorna esclusivamente al blocco sopraggiunto in db ed esce.
	...
	*/
	...
	public function actionScanForTransactions(){
		 // echo '<pre>'.print_r($_POST,true).'</pre>';
 		 // exit;
		set_time_limit(0); //imposto il time limit unlimited

		$latestBlock = $_POST['latest_block'];
		...
		if (!$poaNode){
			$save = new Save;
			$save->WriteLog('wallet-tts','blockchain','scanForTransactions',"All Nodes are down.");
				echo CJSON::encode(array(
					'id'=>time(),
					'error'=>'All Nodes are down.',
					'success'=>false,
		...
		{
			if ((hexdec($savedBlock)+$x) <= hexdec($latestBlock)){
				...
				if (!empty($transactions))
				{
					...
					foreach ($transactions as $transaction)
					{
						...
						if (strtoupper($transaction->to) <> strtoupper($settings->poa_contractAddress) ){
							$ReceivingType = 'ether';
							// if (strtoupper($transaction->to) == $SEARCH_ADDRESS || strtoupper($transaction->from) == $SEARCH_ADDRESS){
							// 	//ricevi o hai inviato
							// 	$tokens = $this->loadTokensByHash($transaction->hash);
							// 	if (null===$tokens){
							// 		fwrite($myfile, "\r\n" .date('Y/m/d h:i:s a', time()) . " : Ether transaction found: <pre>".print_r($transaction,true)."</pre>\n");
							// 		//salva la transazione
							// 		$timestamp = 0;
							// 		$transactionValue = self::hex2eth($transaction->value);
							// 		// if ($transaction->from == $SEARCH_ADDRESS){
							// 		// 	$transactionValue = -$transactionValue;
							// 		// }
							// 		$rate = eth::getFiatRate('eth');
							//
							// 		//recupero il timestamp della transazione
							...
							// 		);
							// 		fwrite($myfile, "\r\n" .date('Y/m/d h:i:s a', time()) . " : Ether transaction save: <pre>".print_r($attributes,true)."</pre>\n");
							...
							// 			//'id' => time(),
							// 			//'success'=>true,
							// 			//'openUrl' => "index.php?r=wallet/history",
							// 			'id_token' => crypt::Encrypt($tokens->id_token),
							// 			'data' => WebApp::dateLN($tokens->invoice_timestamp,$tokens->id_token),
							// 			'status' => WebApp::walletStatus($tokens->status),
							...
							//
							// 		));
							// 		// in caso di ether non invio messaggi nè notifiche
							// 	}else{
							// 		// se non è null verifico che si tratti del ricevente token e non di chi invia
							...
							if ($transactionContract <> '' && !(empty($transactionContract->logs))){
								...
								if (strtoupper($receivingAccount) == $SEARCH_ADDRESS || strtoupper($transactionContract->from) == $SEARCH_ADDRESS){
									//ricevi o hai inviato
									#echo '<pre>'.print_r($transactionContract,true).'</pre>';
									#exit;
									$tokens = $this->loadTokensByHash($transactionContract->transactionHash);

									// SE è null è NECESSARIA LA NOTIFICA per chi invia e riceve
									if (null===$tokens){
										fwrite($myfile, "\r\n" .date('Y/m/d h:i:s a', time()) . " : Token transaction found: <pre>".print_r($transactionContract,true)."</pre>\n");
										//salva la transazione
										$timestamp = 0;
										...
										fwrite($myfile, "\r\n" .date('Y/m/d h:i:s a', time()) . " : Token transaction save: <pre>".print_r($attributes,true)."</pre>\n");
										//salvo la transazione in db. Restituisce object
										$save = new Save;
								 		$tokens = $save->Token($attributes);
										$this->setTransactionFound(array(
											...
											'data' => WebApp::dateLN($tokens->invoice_timestamp,$tokens->id_token),
											'status' => WebApp::walletStatus($tokens->status),
											//'token_price' => WebApp::typePrice($transactionValue,$tokens->type,($transactionValue<0 ? 'sent' : 'received')),
										...
										$notification = array(
											'type_notification' => 'token',
											'id_user' => Wallets::model()->findByAttributes(['wallet_address'=>$tokens->from_address])->id_user,
											'id_tocheck' => $tokens->id_token,
											'status' => 'complete',
		...
		if (!(empty($this->getTransactionFound()))){
			#echo '<pre>Transactions Found'.print_r($this->getTransactionFounded(),true).'</pre>';
			#exit;
			echo CJSON::encode($this->getTransactionFound());
		}else{
			echo CJSON::encode(array(
	 ...
	 */
	 public function actionGetBlockNumber()
	 {
		...
		if (!$poaNode){
			$save = new Save;
			$save->WriteLog('wallet-tts','blockchain','getBlockNumber',"All Nodes are down.");
				echo CJSON::encode(array(
					'id'=>time(),
					'error'=>'All Nodes are down.',
					'success'=>false,
		...
		#echo "<pre>Blocco: ".print_r($response,true)."</pre>";
	...
	private function setTransactionFound($transaction){
		...
		//array_push($this->transactionFounded,$transaction);
		#echo '<pre>Transazione dopo '.print_r($this->getTransactionFounded(),true).'</pre>';
