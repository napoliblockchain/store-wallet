...
class WalletETHController extends Controller
{
	...
	public function actionLoadGAS(){
		...
		if (!$poaNode){
			$save = new Save;
			$save->WriteLog('wallet-tts','walletETH','loadGAS',"All Nodes are down.");
			echo CJSON::encode(array(
	...
	public function actionGetBalance(){
		...
		if (!$poaNode){
			$save = new Save;
			$save->WriteLog('wallet-tts','walletETH','getBalance',"All Nodes are down.");
		}else{
			$web3 = new Web3($poaNode);
			$eth = $web3->eth;
			$balance = 0;
		...
		$send_json = array(
			...
			//'eurbalance' => round(NaPay::getFiatRate('eth') * $balance,2),
			//'id' => crypt::Encrypt($my_address),
			'id'=> $my_address,
		);
	    echo CJSON::encode($send_json);
	}
