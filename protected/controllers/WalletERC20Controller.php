...
class WalletERC20Controller extends Controller
{
	...
	public function actionGetBalance(){
		...
		if (!$poaNode){
			$save = new Save;
			$save->WriteLog('wallet-tts','walletERC20','getBalance',"All Nodes are down.");
		}else{
	 ...
	 */
	 public function actionSend()
	 {
		...
		*/
		...
		if (!$poaNode){
			$save = new Save;
			$save->WriteLog('wallet-tts','walletERC20','send',"All Nodes are down.");
			$time = time();
            ...
            // 			[message] => Nonce too low
        		// 	)
						...
						if ($jsonBody['error']['code'] == -32001){
							...
							echo CJSON::encode(array(
								"error"=>'Error on raw transaction: '.$jsonBody['error']['message'],
								'id'=>time()
							));
							exit;
						}
			...
			$save = new Save;
			// il messaggio push viene ricevuto solo dai wallet tts
			Push::Send($save->Notification($notification),'wallet');
